...
def log2data(infile):
	...
	#
	# By default, the maximum PID value is typically set to
	# 32768 on 32-bit systems and
	# 4194304 on 64-bit systems.
	# You can check the current pid_max value on your system by reading the contents of
	...
	while i < len(lines):
		...
		if items[1] == 'fork' and items[3] == 'parent' and items1[1] == 'fork' and items1[3] == 'child' :
			...
			#if ppid == 294480 and data[pid]['cmd'] != '/sbin/init':
	...
	if mastercmds:
		...
		if 'cmd=All' in mastercmds:
			...
			for mp in mastercmds:
				...
				all_children_positive_map = [0] * 10 * 1000000
...
def makeoptions(cmdpicklist):
	...
	l = []
	l.append('/usr/ccsp/cosa_start.sh')
	...
	options.append('<option value=\"' + '/sbin/init' + '\">' + '/sbin/init' + '</option>')
