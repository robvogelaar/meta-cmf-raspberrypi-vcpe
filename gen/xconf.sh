...
# Create service.properties for xconf-angular-admin
...
# Create service.properties for xconf-dataservice
...
lxc exec ${container_name} -- start-stop-daemon --start --background --make-pidfile \
    --pidfile /var/run/jetty-xconf-admin.pid --chdir /root/xconf/xconfserver/xconf-angular-admin \
...
lxc exec ${container_name} -- start-stop-daemon --start --background --make-pidfile \
    --pidfile /var/run/jetty-xconf-dataservice.pid --chdir /root/xconf/xconfserver/xconf-dataservice \
    --exec /usr/bin/mvn -- jetty:run \
