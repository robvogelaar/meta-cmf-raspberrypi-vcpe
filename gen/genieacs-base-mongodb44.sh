...
lxc exec ${container_name} -- sh -c 'echo "\$nrconf{restart} = '\''a'\'';" > /etc/needrestart/conf.d/50local.conf'
...
lxc exec ${container_name} -- sh -c 'DEBIAN_FRONTEND=noninteractive apt update'
lxc exec ${container_name} -- sh -c 'DEBIAN_FRONTEND=noninteractive NEEDRESTART_MODE=a apt upgrade -y'
lxc exec ${container_name} -- sh -c 'DEBIAN_FRONTEND=noninteractive NEEDRESTART_MODE=a apt install -y curl wget gnupg2 software-properties-common ca-certificates lsb-release'

###################################################################################################################################
# Node.js installation
echo "Installing Node.js 18..."
lxc exec ${container_name} -- sh -c 'curl -fsSL https://deb.nodesource.com/setup_18.x | bash -'
lxc exec ${container_name} -- sh -c 'DEBIAN_FRONTEND=noninteractive NEEDRESTART_MODE=a apt install -y nodejs'
echo "Verifying Node.js installation..."
lxc exec ${container_name} -- node --version
lxc exec ${container_name} -- npm --version

###################################################################################################################################
# MongoDB 4.4 installation (compatible with non-AVX CPUs)
echo "Installing MongoDB 4.4 (compatible with Intel Celeron J3455)..."
# Install libssl1.1 compatibility package first
lxc exec ${container_name} -- sh -c 'wget http://archive.ubuntu.com/ubuntu/pool/main/o/openssl/libssl1.1_1.1.0g-2ubuntu4_amd64.deb'
lxc exec ${container_name} -- sh -c 'DEBIAN_FRONTEND=noninteractive dpkg -i libssl1.1_1.1.0g-2ubuntu4_amd64.deb'
lxc exec ${container_name} -- sh -c 'rm libssl1.1_1.1.0g-2ubuntu4_amd64.deb'
# Install MongoDB 4.4
...
lxc exec ${container_name} -- sh -c 'DEBIAN_FRONTEND=noninteractive apt update'
lxc exec ${container_name} -- sh -c 'DEBIAN_FRONTEND=noninteractive NEEDRESTART_MODE=a apt install -y mongodb-org'
...
echo "Setup complete! The genieacs-base image is ready with GenieACS, MongoDB 4.4, and Node.js."